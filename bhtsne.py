...
def getFloat(source):
    try:
        ...
        code = int(hashlib.sha1(source).hexdigest(), 16)
        # print ('converted ' + source + ' to ' + str(code))
        return code
...
def init_bh_tsne(input_file, workdir, no_dims=DEFAULT_NO_DIMS, initial_dims=INITIAL_DIMENSIONS, perplexity=DEFAULT_PERPLEXITY,
            theta=DEFAULT_THETA, randseed=EMPTY_SEED, verbose=False, use_pca=DEFAULT_USE_PCA, max_iter=DEFAULT_MAX_ITERATIONS, lock=mp.Lock()):
    lock.acquire()
    print('loading data started\r\n')
    # Read the data, with some sanity checking
    # Skip the last data - the label
    samples = []
    tags = []
    lines = 0
    for sample_line_num, sample_line in enumerate((l.rstrip('\n')
        ...
        samples.append([getFloat(e) for e in sample_data][:-1])
        tags.append([getFloat(e) for e in sample_data][-1])

    samples = np.asarray(samples, dtype='float64')
    tags = np.asarray(tags, dtype='float64')
    ...
    # with open(path_join(workdir, 'tags.dat'), 'wb') as data_file:
    #     for tag in tags:
    #         data_file.write(pack('{}d'.format(len(sample)), *sample))
    lock.release()
    return tags
...
def run_bh_tsne(input_file, no_dims=2, perplexity=50, theta=0.5, randseed=-1, verbose=False,initial_dims=50, use_pca=True, max_iter=1000):
    ...
    lock = mp.Lock()
    # mp.Process(target=init_bh_tsne, args=(input_file, tmp_dir_path, no_dims, perplexity, theta, randseed,verbose, initial_dims, use_pca, max_iter, lock)).start()
    # time.sleep(1)
    # print('waiting for loading process\r\n')
    # lock.acquire()

    tags = init_bh_tsne(input_file, tmp_dir_path, no_dims, perplexity, theta, randseed,verbose, initial_dims, use_pca, max_iter, lock)
    ...
    rmtree(tmp_dir_path)
    return np.asarray(res, dtype='float64'), tags


def main(args):
    argp = _argparse().parse_args(args[1:])
    results, tags = run_bh_tsne(argp.input, no_dims=argp.no_dims, perplexity=argp.perplexity, theta=argp.theta, randseed=argp.randseed,
    ...
    fig = plt.figure(0)
    fig.suptitle('perplexity ', fontsize=14, fontweight='bold')
    plt.scatter(results[:,0], results[:,1], s=100, c=tags, alpha=0.5)
    image = './images/tsne_{}_{}_{}.png'.format(str(datetime.datetime.now().day),str(datetime.datetime.now().hour), str(datetime.datetime.now().minute))
    plt.savefig(image)
